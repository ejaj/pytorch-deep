...
def figure7(X, y, model, device, cm=None, cm_bright=None):
    ...
    surf = ax.plot_surface(xx, yy, yhat, rstride=1, cstride=1, alpha=.5, cmap=cm, linewidth=0, antialiased=True, vmin=0,
                           vmax=1)
    # Plot the training points
    ax.scatter(X[:, 0], X[:, 1], c=y, cmap=cm_bright)
    # Plot the testing points
...
def figure20(y):
    ...
    fig = eval_curves(fpr_perfect, tpr_perfect, rec_perfect, prec_perfect, thresholds1_perfect, thresholds2_perfect,
                      line=True)
    return fig
...
def figure21(y, probabilities):
    ...
    fig = eval_curves(fpr_random, tpr_random, rec_random, prec_random, thresholds1_random, thresholds2_random,
                      line=True)
    axs = fig.axes
