...
def counter_vs_clock(basic_corners=None, basic_colors=None, basic_letters=None, draw_arrows=True, binary=True):
    ...
    for is_clock in range(1 + draw_arrows):
        ...
        for i in range(4):
            ...
            axs[is_clock].text(*(coords - factor * np.array([.05, 0.05])), letter, c='k' if i in targets else 'w',
                               fontsize=12)
    ...
    for i in range(n_points):
        ...
        for j, layer in enumerate(layers):
            ...
            if layer in gates.keys():
                axs[i][j + 1].set_title(
                    titles[j + 1][:-1] + '\ [' + ','.join(['{:.2f}'.format(v) for v in gates[layer][i]]) + ']$')
            else:
...
if __name__ == '__main__':
    ...
    n_lines = 11
    n_points = 1000
    grid = build_2d_grid(xlim, ylim, n_lines, n_points)
