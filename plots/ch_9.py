...
def get_angle_plot(line1, line2, offset=1, color=None, origin=(0, 0), len_x_axis=1, len_y_axis=1):
    ...
    angle_plot = Arc(origin, len_x_axis * offset, len_y_axis * offset, 0, theta1, theta2, color=color,
                     label=str(angle) + u"\u00b0")
...
def project_cosine(q, k):
    ...
    ax = axs[1]
    add_arrow(make_line(ax, q / norm_q), lw=2, color='r', text=r'$||\hat{Q}||=1$', size=12, alpha=.9,
              text_offset=(.02, .02))
    add_arrow(make_line(ax, k / norm_k), lw=2, color='k', text=r'$||\hat{K}||=1$', size=12, alpha=.9,
              text_offset=(.02, .02))
    ...
    add_arrow(make_line(ax, unit_proj), lw=2, color='r',
              text=r'$||\hat{Q}_\hat{K}||=cos\theta=' + f'{cos_theta:.2f}' + '$', size=12, alpha=.9,
              text_offset=(-.35, -.16))
...
def project_cosine_scaling(q, k):
    ...
    add_arrow(make_line(ax, unit_proj * norm_q), lw=2, color='r',
              text=r'$cos\theta\ ||Q||=' + f'{cos_theta * norm_q:.2f}' + '$', size=12, alpha=.9,
              text_offset=(-.18, .03))
...
def query_and_keys(q, ks, result=None, ax=None):
    ...
    add_arrow(line_q, lw=2, color='r', text=f'||Q||={norm_q:.2f}', size=12)
    add_arrow(line_k[0], lw=2, color='k',
              text=r'$||K_0' + f'||={norm_k[0]:.2f}$' + '\n' + r'$cos\theta_0=' + f'{cos_k[0]:.2f}$', size=12,
              text_offset=(-.33, .1))
    add_arrow(line_k[1], lw=2, color='k',
              text=r'$||K_1' + f'||={norm_k[1]:.2f}$' + '\n' + r'$cos\theta_1=' + f'{cos_k[1]:.2f}$', size=12,
              text_offset=(-.63, -.18))
    add_arrow(line_k[2], lw=2, color='k',
              text=r'$||K_2' + f'||={norm_k[2]:.2f}$' + '\n' + r'$cos\theta_2=' + f'{cos_k[2]:.2f}$', size=12,
              text_offset=(.05, .58))
...
def plot_attention(model, inputs, point_labels=None, source_labels=None, target_labels=None, decoder=False,
                   self_attn=False, n_cols=5, alphas_attr='alphas'):
...
def encoding_degrees(dims, seqs, tot):
    ...
    for dim in range(dims):
        xs, ys = np.cos(np.linspace(0, tot / seqs[dim], tot + 1) * 2 * np.pi).reshape(-1, 1), np.sin(
            np.linspace(0, tot / seqs[dim], tot + 1) * 2 * np.pi).reshape(-1, 1)
