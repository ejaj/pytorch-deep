...
class FeatureSpace(Basic):
    ...
    def _update(i, fs, epoch_start=0, colors=None, **kwargs):
        ...
        if not fs.scale_fixed:
            xlim = [fs.bent_contour_lines[epoch, :, :, 0].min() - .05,
                    fs.bent_contour_lines[epoch, :, :, 0].max() + .05]
            ylim = [fs.bent_contour_lines[epoch, :, :, 1].min() - .05,
                    fs.bent_contour_lines[epoch, :, :, 1].max() + .05]
            fs.ax.set_xlim(xlim)
            fs.ax.set_ylim(ylim)
...
def build_decision_boundary(model, states, X, y, layer_name=None, contour_points=1000, xlim=(-1, 1), ylim=(-1, 1),
                            display_grid=True,
